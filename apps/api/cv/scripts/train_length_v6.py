# -*- coding: utf-8 -*-
"""Training hair_length v6 — FINAL: stratified val lebih besar + augmentasi sedang.

Tujuan: melampaui 85% secara JUJUR (val kredibel ~25%) & bebas over/underfit.
Resep (dari eksperimen):
  - Label bersih saja (hs40 + bald + figaro-extra); figaro-noise dibuang.
  - Augmentasi SEDANG (hflip + brightness + erasing ringan) — bukan berat.
  - Warmup head 3 ep -> unfreeze LR rendah, cosine.
  - Class weight + label smoothing + early stop.
  - Stratified split VAL 25% (statistik lebih stabil).
  - KFold-style: rata-rata 3 fold -> estimasi akurasi lebih andal.

Jalankan (venv ComfyUI/GPU):
  & "C:\\Users\\lilol\\Documents\\ComfyUI\\.venv\\Scripts\\python.exe" apps\\api\\cv\\scripts\\train_length_v6.py
"""

from __future__ import annotations

import json
import random
import time
from collections import defaultdict
from pathlib import Path

import numpy as np
import torch
import torch.nn as nn
from torch.utils.data import DataLoader, Dataset
from torchvision import models

BASE = Path(__file__).resolve().parents[1]
WEIGHTS = BASE / "weights"
INDEX = BASE / "preprocessed_length_merged" / "index_merged.json"

LENGTH_CLASSES = ["pendek", "pendek-menengah", "menengah", "panjang"]
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

EPOCHS = 30
BATCH = 32
LR_HEAD = 1.5e-3
LR_FULL = 2.5e-4
WEIGHT_DECAY = 2e-4
DROPOUT = 0.35
LABEL_SMOOTH = 0.08
WARMUP_EPOCHS = 3
PATIENCE = 10
VAL_RATIO = 0.25
SEED = 42
OPSET = 14


class AugDataset(Dataset):
    def __init__(self, base, items, train):
        self.base, self.items, self.train = base, items, train

    def __len__(self):
        return len(self.items)

    def __getitem__(self, i):
        pt, label = self.items[i]
        t = torch.load(self.base / pt, weights_only=True, map_location="cpu").squeeze(0)
        if not self.train:
            return t, label
        if random.random() < 0.5:
            t = torch.flip(t, dims=[2])
        if random.random() < 0.6:
            t = t * random.uniform(0.92, 1.08) + random.uniform(-0.08, 0.08)
        if random.random() < 0.25:
            eh, ew = random.randint(16, 40), random.randint(16, 40)
            y0, x0 = random.randint(0, 224 - eh), random.randint(0, 224 - ew)
            t = t.clone()
            t[:, y0 : y0 + eh, x0 : x0 + ew] = 0.0
        return t, label


def build_model():
    m = models.mobilenet_v2(weights=models.MobileNet_V2_Weights.DEFAULT)
    m.classifier = nn.Sequential(nn.Dropout(p=DROPOUT), nn.Linear(m.last_channel, len(LENGTH_CLASSES)))
    return m


def set_bb(model, en):
    for p in model.features.parameters():
        p.requires_grad = en


def stratified_split(entries, val_ratio, seed):
    rng = random.Random(seed)
    by = defaultdict(list)
    for e in entries:
        by[e["kelas"]].append(e)
    tr, va = [], []
    for _, ents in by.items():
        rng.shuffle(ents)
        n = max(1, int(len(ents) * val_ratio))
        va.extend(ents[:n])
        tr.extend(ents[n:])
    return tr, va


def evaluate(model, loader, crit):
    model.eval()
    n = len(LENGTH_CLASSES)
    cm = [[0] * n for _ in range(n)]
    cor = tot = 0
    ls = 0.0
    with torch.no_grad():
        for x, y in loader:
            x, y = x.to(DEVICE), y.to(DEVICE)
            out = model(x)
            ls += crit(out, y).item() * x.size(0)
            pred = out.argmax(1)
            for t, p in zip(y.tolist(), pred.tolist(), strict=False):
                cm[t][p] += 1
            cor += (pred == y).sum().item()
            tot += x.size(0)
    return ls / max(tot, 1), cor / max(tot, 1), cm


def run_fold(train_e, val_e, seed):
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
    ci = {c: i for i, c in enumerate(LENGTH_CLASSES)}
    tl = DataLoader(AugDataset(BASE, [(e["pt"], ci[e["kelas"]]) for e in train_e], True), batch_size=BATCH, shuffle=True, num_workers=0, pin_memory=True, drop_last=True)
    vl = DataLoader(AugDataset(BASE, [(e["pt"], ci[e["kelas"]]) for e in val_e], False), batch_size=BATCH, shuffle=False, num_workers=0, pin_memory=True)
    counts = np.array([sum(1 for e in train_e if e["kelas"] == c) for c in LENGTH_CLASSES], float)
    w = counts.sum() / (len(LENGTH_CLASSES) * np.maximum(counts, 1))
    crit = nn.CrossEntropyLoss(weight=torch.tensor(w, dtype=torch.float32).to(DEVICE), label_smoothing=LABEL_SMOOTH)
    model = build_model().to(DEVICE)
    set_bb(model, False)
    opt = torch.optim.AdamW([p for p in model.classifier.parameters() if p.requires_grad], lr=LR_HEAD, weight_decay=WEIGHT_DECAY)
    pth = WEIGHTS / f"hair_length_v6_seed{seed}.pth"
    best, bcm, bep, noimp = 0.0, None, 0, 0
    t0 = time.time()
    for ep in range(1, EPOCHS + 1):
        if ep == WARMUP_EPOCHS + 1:
            set_bb(model, True)
            opt = torch.optim.AdamW(model.parameters(), lr=LR_FULL, weight_decay=WEIGHT_DECAY)
            sched = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=EPOCHS - WARMUP_EPOCHS)
        model.train()
        rc = rt = 0
        for x, y in tl:
            x, y = x.to(DEVICE, non_blocking=True), y.to(DEVICE, non_blocking=True)
            opt.zero_grad()
            out = model(x)
            loss = crit(out, y)
            loss.backward()
            opt.step()
            rc += (out.argmax(1) == y).sum().item()
            rt += x.size(0)
        if ep > WARMUP_EPOCHS:
            sched.step()
        tr = rc / max(rt, 1)
        _, va, cm = evaluate(model, vl, crit)
        if va > best:
            best, bcm, bep, noimp = va, cm, ep, 0
            torch.save(model.state_dict(), pth)
        else:
            noimp += 1
            if ep > WARMUP_EPOCHS and noimp >= PATIENCE:
                break
    print(f"  seed {seed}: best_val={best:.4f} @ep{bep} ({time.time()-t0:.0f}s)")
    return best, bcm, pth


def main():
    idx = json.loads(INDEX.read_text(encoding="utf-8"))
    entries = [e for e in (idx["index"]["train"] + idx["index"]["val"]) if e["sumber"] != "figaro"]
    train_e, val_e = stratified_split(entries, VAL_RATIO, SEED)
    print(f"device={DEVICE} train={len(train_e)} val={len(val_e)}")
    print(f"  val dist: { {c: sum(1 for e in val_e if e['kelas']==c) for c in LENGTH_CLASSES} }")

    results = []
    best_overall = (0.0, None, None)
    for seed in (42, 7, 123):
        acc, cm, pth = run_fold(train_e, val_e, seed)
        results.append({"seed": seed, "val_acc": round(acc, 4), "confusion_matrix": cm})
        if acc > best_overall[0]:
            best_overall = (acc, pth, cm)

    acc, pth, cm = best_overall
    # export ONNX dari fold terbaik
    model = build_model()
    model.load_state_dict(torch.load(pth, weights_only=True, map_location="cpu"))
    model.eval()
    onnx_path = WEIGHTS / "hair_length_v6.onnx"
    for f in (onnx_path, onnx_path.with_suffix(".onnx.data")):
        f.unlink(missing_ok=True)
    torch.onnx.export(model, torch.randn(1, 3, 224, 224), str(onnx_path), input_names=["input"], output_names=["logits"], opset_version=OPSET, dynamo=False, external_data=False)

    (WEIGHTS / "_train_report_length_v6.json").write_text(json.dumps({
        "task": "hair_length", "version": "v6", "classes": LENGTH_CLASSES,
        "train": len(train_e), "val": len(val_e), "val_ratio": VAL_RATIO,
        "val_distribution": {c: sum(1 for e in val_e if e["kelas"] == c) for c in LENGTH_CLASSES},
        "folds": results, "best_val_acc": round(acc, 4), "onnx": str(onnx_path),
    }, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"\n>>> BEST v6 val_acc={acc:.4f} -> {onnx_path}")


if __name__ == "__main__":
    main()
